    ...
    try {
      ...
      const handleScroll = () => {
        ...
        if (vialGroupRef.current) {
          vialGroupRef.current.position.z = scrollProgress * 1.5;
          vialGroupRef.current.scale.setScalar(1 + scrollProgress * 0.2);
        }
      };
      ...
      const animate = () => {
        ...
        // Rotate vial slowly
        if (vialGroupRef.current) {
          vialGroupRef.current.rotation.y += mobile ? 0.003 : 0.005;
          
          // Mouse parallax effect (desktop only)
          if (!mobile) {
            vialGroupRef.current.rotation.x = Math.PI * 0.05 + mouseRef.current.y * 0.08;
            vialGroupRef.current.rotation.z = mouseRef.current.x * 0.03;
          }
        }
